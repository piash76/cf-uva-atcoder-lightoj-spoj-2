...
int main()
{
    ...
    {
        pref[i]=1;
        if(i>1 and a[i-1]<a[i]) pref[i]+=pref[i-1];
        ans=max(ans,pref[i]);
    }
    ...
    {
        suf[i]=1;
        if(i+1<=n and a[i]<a[i+1]) suf[i]+=suf[i+1];
        ans=max(ans,suf[i]);
    }
    ...
    for(i=1;i<=n;i++)
    {
        if(i<n) ans=max(ans,pref[i]+1);
        if(i>1) ans=max(ans,suf[i]+1);

        if(i>1 and i<n and a[i+1]-a[i-1]>1) ans=max(ans,pref[i-1]+1+suf[i+1]);
    }
