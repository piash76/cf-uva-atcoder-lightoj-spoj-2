...
int main()
{
   ...
   cin>>t;
    while(t--)
    {
        ll n;cin>>n;
        ll ma=-1;
        f2(i,1,n) { cin>>a[i];pre[i]=a[i];ma=max(ma,a[i]);}



        f2(i,2,n) pre[i]+=pre[i-1];
        ...
        f2(i,1,n)
        {
            ...
            if(pre[i]==x)
            {
                ll i1=n-i;
                ll y=(i1*(i1+1))/2;
               // cout<<pre[n]-pre[i]<<" "<<y<<endl;
                if(pre[n]-pre[i]==y)
                {

                    int f=1;
                    for(j=1;j<=n;j++) cnt[j]=0;
                    for(j=1;j<=i;j++) cnt[a[j]]=1;


                    for(j=1;j<=i;j++)
                    {
                      //  cout<<cnt[j]<<" ";
                        if(cnt[j]==0) {f=0;break;}
                    }
                   // cout<<endl;

                    for(j=1;j<=n;j++) cnt[j]=0;
                    for(j=i+1;j<=n;j++) cnt[a[j]]=1;
