...
ll a[maxn],mn[maxn],mx[maxn],cnt[2*maxn+5];
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(i=0;i<n/2;i++)
        {
            ll x=a[i]+a[n-i-1];
            cnt[x]++;
            mn[i]=min(a[i],a[n-i-1])+1;
            mx[i]=max(a[i],a[n-i-1])+k;
        }

        sort(mn,mn+n/2);sort(mx,mx+n/2);
      // for(i=0;i<n/2;i++) cout<<mn[i]<<gp<<mx[i]<<endl;
        ll ans=LLONG_MAX;

        for(i=2;i<=2*k;i++)
        {
            //cout<<i<<endl;
            ll ind1=upper_bound(mn,mn+n/2,i)-mn;
            ll ind2=lower_bound(mx,mx+n/2,i)-mx;

            ll m1=ind1-ind2;   /// one or zero replace
           // cout<<i<<gap<<m1<<endl;
            ll on=m1-cnt[i]; /// one replace
            //cout<<i<<gap<<m1<<endl;

            ll m2=2*((n/2)-m1);  /// two replace

            //cout<<i<<gp<<on<<gp<<m2<<endl;
