...
int main()
{
    ...
    f1(i,0,n)
    {
        f1(j,1,m)
        {
            if(a[i][j]<=a[i][j-1] and b[i][j]<=b[i][j-1]) {flag=0;break;} ///gut
            if( (a[i][j-1]>a[i][j] or b[i][j-1]>b[i][j] ) and (b[i][j-1]<a[i][j] and a[i][j-1]<b[i][j]) ) { swap(a[i][j],b[i][j]);}
        }
    }
    if(flag==0) { cout<<"Impossible"<<endl;return 0; }
    f1(i,0,n)
    {
        f1(j,1,m)
        {
            if(a[j][i]<=a[j-1][i] and b[j][i]<=b[j-1][i]) {cout<<"eikhane "<<a[j][i]<<" "<<a[j-1][i]<<endl;flag=0;break;} ///gut
            if ( (a[j][i-1]>a[j][i] or b[j][i-1]>b[j][i] ) and (b[j-1][i]<a[j][i] and a[j-1][i]<b[j][i]) ) swap(a[j][i],b[j][i]);
        }
    }
    ...
    f1(i,0,n)
    {
        ...
        {
            //cout<<a[i][j]<<" ";
            if(a[i][j]<=a[i][j-1] or b[i][j]<=b[i][j-1]) { flag=0;break; }
        }
       // cout<<endl;
    }
    f1(i,0,n)
    {
        ...
        {
            //cout<<b[i][j]<<" ";
            if(a[j][i]<=a[j-1][i] or b[j][i]<=b[j][i-1])   { flag=0; break; }
        }
        //cout<<endl;
    }
