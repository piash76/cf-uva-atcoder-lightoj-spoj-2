...
void dfs(ll s)
{
    ...
    for(auto x:g[s])
    {
       ...
       {
           dfs(x);
           len[s]+=len[x];///calculating sub length
           if(len[x]>n/2) is_centroid=false;
       }
    }
    if(n-len[s]>n/2) is_centroid=false;

    if(is_centroid) centroid.push_back(s);
}
